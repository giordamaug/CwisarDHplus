...
int main(int argc,char **argv) {
    ...
    // train globals
    int pretrainstart=1, pretrainend=1, teststart, testend;
    ...
    if (overtrain == false) {
        ...
        if (troifile.is_open()) {
            ...
            while ( getline (troifile,line) ) {
               sscanf(line.c_str(), "%d %d", &pretrainend, &testend);
               teststart = pretrainend;
            }
            troifile.close();
        } else {
...
#pragma omp parallel for schedule(dynamic,1) shared(den,denD,neigh_map,neigh_mapD,wiznet,wiznetD,threshold,thresholdD,width,height,nt,ntD,frame,greyframeD,greyoutframe,greyoutframeD,queuesize,queuesizeD,roiimg) private(sum,sumD,neuron,i,j,discr,discrD,cache,cacheD,queue,queueD,p, data,dataD,odata,odataD,roidata,R,G,B,cvalD)
            ...
            if (xdispflag) {
                ...
                cvShowManyImages(title, stream.str(), teststart, testend, wait, status, tp, fp, fn, tn, nbShadowErrors, recall, precision, (tn / (double)(tn + fp)), (2.0 * (recall * precision) / (recall + precision)), fps, imglist);
            }
