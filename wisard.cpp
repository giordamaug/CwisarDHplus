...
wisard_t *net_create(int n_bit, int im_width, int im_height, string colormode, int nt, int cachesize, int queuesize) {
    register int i, j;
    int npixels;
    ...
    if (colormode == "RGB") {
        ...
        // compute number of rams
        if (n->nt % n_bit == 0)
            n->n_ram = (int)(n->nt / n_bit);
        else
            n->n_ram = (int)(n->nt / n_bit) + 1;
        // compute neighbour mapping
        n->neigh_map = neigh_mapping(n->n_ram, n->n_bit, n->nt, 1);
        n->neigh_w = 1;
        n->neigh_h = nt;
    } else {
    ...
    // alloc queue
    queue_entry_t **queue;
    queue = (queue_entry_t **)malloc(npixels * sizeof(queue_entry_t *));
    for (i = 0; i < npixels; i++) {
        net[i] = (wentry_t**)malloc(n->n_ram * sizeof(wentry_t*));
        cache[i] = (cache_entry_t *)cacheinit(n->n_ram, cachesize);
        queue[i] = (queue_entry_t *)queueinit(n->n_ram, queuesize);
        for (j=0; j < n->n_ram; j++) {
            net[i][j] = wram_create();
        }
    }
...
pix_t **neigh_mapping(int n_ram, int n_bit, int w, int h) {
    ...
    for (i = 0; i < n_ram; i++)
        for (j=0; j < n_bit; j++)
            pixels[i * n_bit + j] = newpix((long int)i,mypowers[j]);
...
queue_entry_t *queueinit(int nram, int queuesize) {
    queue_entry_t *top, *newp;
    register int neuron;
    // create first element;
    newp = (queue_entry_t *)malloc(sizeof(queue_entry_t));
    newp->index = 0;
    newp->tuple = (int *)malloc(nram * sizeof(int));
    for (neuron=0;neuron<nram;neuron++) newp->tuple[neuron] = 0;
    newp->next = newp;
    newp->prev = newp;
    top = newp;
...
// Queue management functions
void queueempty(queue_entry_t *queue, int nram) {
    queue_entry_t *p;
    register int neuron;
    
    // add other element;
    for (p = queue;p->next != queue;p=p->next) {
        p->index = 0;
        for (neuron=0;neuron<nram;neuron++) p->tuple[neuron] = 0;
    }
    return;
}

// pixels utility functions
pix_t newpix(long int x,long int y) {
    pix_t px = {x, y};
    return px;
